...
class DetectionLogic(threading.Thread):
    ...
    def _draw_bounding_box(self, frame, bbox, label_text):
        #Fungsi untuk menggambar bounding box pada frame Visual indicator untuk area kode yang terdeteksi
    ...
    def _send_bbox_update(self, frame, bbox, code):
        #Tampilkan bbox langsung setelah deteksi tanpa menunggu frame berikutnya
        try:
    ...
    def _process_and_send_frame(self, frame, is_static):
        ...
        #Check apakah bbox sudah expired (lebih dari bbox_display_duration detik)
    ...
    def _normalize_din_code(self, code):
        ...
        #Pattern 3: LN + digit + kapasitas angka + huruf suffix + ISS, Contoh: LN4776AISS -> "LN4 776A ISS"
        match = re.match(r'^(LN\d)(\d+)([A-Z])(ISS)$', code_no_space)
        if match:
            return f"{match.group(1)} {match.group(2)}{match.group(3)} {match.group(4)}"
        
        #Pattern 4: LN + digit + kapasitas angka + huruf suffix, Contoh: LN4776A -> "LN4 776A", LN1450A -> "LN1 450A"
        match = re.match(r'^(LN\d)(\d+)([A-Z])$', code_no_space)
        if match:
            return f"{match.group(1)} {match.group(2)}{match.group(3)}"
        
        #Pattern 5: LN + digit + kapasitas angka saja (tanpa huruf suffix), Contoh: LN2360 -> "LN2 360"
        match = re.match(r'^(LN\d)(\d+)$', code_no_space)
        if match:
            return f"{match.group(1)} {match.group(2)}"
        
        # Pattern 6: sudah punya spasi yang benar - normalize saja, Pastikan format "LN4 776A ISS" konsisten
    ...
    def _correct_din_structure(self, text):
        ...
        #Reverse dengan LN terbaca sebagai typo (1N, IN, LH)
        ...
        #Insert spasi dan koreksi per-token
    ...
    def _find_best_din_match(self, detected_text):
        ...
        #Single pass - semua pengecekan dalam 1 loop
    ...
    def _correct_jis_structure(self, text):
        ...
        dan konversi huruf mirip-angka di posisi numerik (capacity & size) menjadi digit.
        Format JIS: [2-3 digit][1 HURUF A-H][2-3 digit][L/R optional][(S) optional]
        """
        #Trim dan uppercase text
        text = text.strip().upper().replace(' ', '')
        
        #Dictionary mapping angka yang sering salah dibaca ke huruf (untuk posisi middle_char)
        digit_to_letter = {
            '0': 'D', '1': 'I', '2': 'Z', '3': 'B', 
            '4': 'A', '5': 'S', '6': 'G', '8': 'B',
        }
        
        #Dictionary mapping huruf mirip-angka ke digit (untuk posisi capacity & size)
        #KUNCI: O->0 adalah akar masalah "36B2OR" dimana O adalah huruf bukan angka 0
        letter_to_digit = {
            'O': '0', 'Q': '0',  #Mirip angka 0
            'I': '1', 'L': '1',  #Mirip angka 1
            'Z': '2',  #Mirip angka 2
            'S': '5',  #Mirip angka 5
            'G': '6',  #Mirip angka 6
            'B': '8',  #Mirip angka 8
        ...
        #Pisahkan optional suffix (S) dulu
        option = ''
        main_text = text
        ...
        #Pisahkan terminal L/R di akhir
        terminal = ''
        if main_text and main_text[-1] in ['L', 'R']:
            terminal = main_text[-1]
            main_text = main_text[:-1]
        
        #Scan posisi middle_char (huruf A-H) dari posisi 2 dan 3
        #Format JIS: [2-3 char capacity][1 char A-H middle][2-3 char size]
        if len(main_text) >= 5:
            for mid_pos in [2, 3]:
                if mid_pos < len(main_text):
                    potential_mid = main_text[mid_pos]
                    
                    #Cek jika middle_char adalah huruf valid A-H
                    if potential_mid in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']:
                        raw_cap = main_text[:mid_pos]
                        mid_char = potential_mid
                        raw_size = main_text[mid_pos+1:]
                        
                        #Koreksi capacity & size: konversi huruf mirip-angka ke digit
                    ...
                    #Cek jika middle_char adalah digit yang bisa dikoreksi ke huruf A-H
        ...
        #Fallback ke pattern original JIS (hanya digit)
        ...
        if match:
            ...
            #Koreksi middle_char jika digit
    ...
    def _find_best_jis_match(self, detected_text):
        """
        Mencari match terbaik dari JIS_TYPES dengan (S) preserved
        Fuzzy matching dengan daftar JIS_TYPES, handle (S) option secara khusus
        """
        #Koreksi struktural terlebih dahulu
        ...
        #Exact match langsung setelah koreksi (optimasi cepat)
        ...
        # Jika tidak ada match yang cukup baik ATAU score < 0.90, Coba matching tanpa (S) untuk handle case dimana (S) detection tidak reliable
    ...
    def scan_frame(self, frame, is_static=False, original_frame=None):
        """
        OCR mentah dengan bounding box detection
        Structural correction + Fuzzy matching
        """
        ...
        if not is_static:
            ...
            if self.split_mode:
                ...
                #Apply edge detection untuk top frame
        ...
        try:
            ...
            #Grayscale dasar - cocok untuk label kontras tinggi
            processing_stages['Grayscale'] = gray
            
            #Sharpened - mempertajam teks yang buram/tidak fokus
            processing_stages['Sharpened'] = cv2.filter2D(gray, -1, kernel_sharpen)
            
            #OTSU - auto-threshold, menggantikan Binary_11/21 dan OTSU_Inv
            _, otsu_frame = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            processing_stages['OTSU'] = otsu_frame
            
            #CLAHE (Contrast Limited Adaptive Histogram Equalization) - untuk label kusam, kuning, pencahayaan tidak merata
            ...
            for stage_name, processed_frame in processing_stages.items():
                try:
                    ...
                    #Early exit jika sudah ada hasil dengan confidence tinggi
            ...
            #Mengelompokkan berdasarkan tempatnya agar terlihat pola pada DIN=====
            ...
            if best_match:
                ...
                #Clear bbox jika tidak ada deteksi
                self.last_detected_bbox = None
                self.last_detected_code = None
    ...
    def start_detection(self):
        # Fungsi untuk start detection atau Mulai live camera detection
        if self.running:
            return 
    ...
    def stop_detection(self):
        # Fungsi untuk stop detection atau Hentikan live camera detection    
        self.running = False #Set flag running False (akan stop loop di run())
        
        #Clear bounding box saat stop
        self.last_detected_bbox = None
        self.last_detected_code = None
    ...
    def set_camera_options(self, preset, flip_h, flip_v, edge_mode, split_mode, scan_interval):
        # Fungsi untuk setting camera options dari UI Update settings
    ...
    def set_target_label(self, label):
        # Fungsi untuk Set label yang sedang dideteksi untuk validasi OK/Not OK
        self.target_label = label

    def check_daily_reset(self):
        # Fungsi untuk check Reset data detected_codes setiap ganti hari
    ...
    def scan_file(self, filepath):
        # Fungsi untuk scan static image file atau OCR detection dari file gambar (bukan live camera)
    ...
    def delete_codes(self, record_ids):
        # Fungsi untuk delete deteksi berdasarkan ID atau Hapus record dari database dan local list
